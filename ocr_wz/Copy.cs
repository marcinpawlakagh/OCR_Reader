...
namespace ocr_wz
{
    ...
    public class Copy
    {
        ...
        public void CopyWZdeliveryDoc()
        {
            ...
            try
            {
                File.Copy(pdfName, Config.outPath + "Rok_20" + year + "\\" + docName + "_DD.pdf");
                StreamWriter WZ;
                WZ = File.AppendText(fileLog);
                WZ.WriteLine(Config.outPath + "Rok_20" + year + "\\" + docName + "_DD.pdf");
                WZ.Close();
            }
            catch (IOException)
            {
                FileInfo infoFirst = new FileInfo(Config.outPath + "Rok_20" + year + "\\" + docName + "_DD.pdf");
                long byteFirst = infoFirst.Length;
                FileInfo infoSecond = new FileInfo(pdfName);
                long byteSecond = infoSecond.Length;
                if (byteFirst != byteSecond)
                {
                    ...
                    StreamWriter WZ2;
                    WZ2 = File.AppendText(fileLog);
                    WZ2.WriteLine("!! DUPLIKAT !!    " + Config.outPath + "Rok_20" + year + "\\do_polaczenia\\" + docName + "_DD.pdf" + "    !! DUPLIKAT !!");
                    WZ2.Close();
                }
                else
                {
                    StreamWriter WZ3;
                    WZ3 = File.AppendText(fileLog);
                    WZ3.WriteLine("!! Identyczny plik istnieje !!    " + Config.outPath + "Rok_20" + year + "\\" + docName + "_DD.pdf" + "    !! Identyczny plik istnieje !!");
                    WZ3.Close();
                }
            }
        }
